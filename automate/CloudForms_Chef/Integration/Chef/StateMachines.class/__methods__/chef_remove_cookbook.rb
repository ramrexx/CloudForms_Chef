=begin
 chef_remove_cookbook.rb

 Author: Kevin Morey <kevin@redhat.com>, Dave Costakos <david.costakos@redhat.com>

 Description: This method uses knife to remove a cookbook from a Chef client
-------------------------------------------------------------------------------
...
begin
  ...
  if bootstrapped =~ (/(true|t|yes|y|1)$/i)
    remove_cookbook_cmd  = "/usr/bin/knife node run_list remove #{chef_node_name} recipe[cookbook::#{chef_cookbook}] "
    remove_cookbook_cmd += "-E #{chef_environment} -F json "
    remove_cookbook_result = call_chef(remove_cookbook_cmd, 20)

    if remove_cookbook_result.success?
      log(:info, "Cookbook #{chef_cookbook} remove successful", true)
      update_vm_custom_attributes(remove_cookbook_result.output, chef_node_name)
    else
      raise "Cookbook #{chef_cookbook} remove failed"
    end
  end
