=begin
 ...
 Description: This method uses knife to add a recipe to a Chef client
-------------------------------------------------------------------------------
...
begin
  ...
  if bootstrapped =~ (/(true|t|yes|y|1)$/i)
    add_recipe_cmd  = "/usr/bin/knife node run_list add #{chef_node_name} recipe[#{chef_recipe}] "
    add_recipe_cmd += "-E #{chef_environment} -F json "
    add_recipe_result = call_chef(add_recipe_cmd, 20)

    if add_recipe_result.success?
      log(:info, "Recipe #{chef_recipe} add successful", true)
      update_vm_custom_attributes(add_recipe_result.output, chef_node_name)
    else
      raise "Recipe #{chef_recipe} add failed"
    end
  end
