=begin
 ...
 Description: This method uses knife to remove a role from a Chef client
-------------------------------------------------------------------------------
...
begin
  ...
  if bootstrapped =~ (/(true|t|yes|y|1)$/i)
    ...
    remove_role_cmd  = "/usr/bin/knife node run_list remove #{chef_node_name} role[#{chef_role}] "
    remove_role_cmd += "-E #{chef_environment} -F json "
    remove_role_result = call_chef(remove_role_cmd, 20)

    if remove_role_result.success?
      log(:info, "Role #{chef_role} remove successful", true)
      update_vm_custom_attributes(remove_role_result.output, chef_node_name)
    else
      raise "Role #{chef_role} remove failed"
    end
  end
